...
1. init() 智能缩减 num
   ...
   → Lab：不缩减num
   → 保存原始num到originalNum字段（用于原料计算）
   → 示例：stackLayers=4
     - 设备num=60 → 缩减为15，originalNum=60
     - Lab num=4 → 保持4，无originalNum

2. generateBuildings() 单层布局
   → 所有设备在z=0层布局
   → Lab垂直堆叠到maxLabLayers

3. generateConveyorBelts() rate不放大
   → rate保持原始值（不再×stackLayers）
   → 使用originalNum计算原料需求
   → 分拣器分配：sortersPerNode = max(2, floor((max-1)/stackLayers))

4. cloneToStackLayers() 智能克隆
   → 克隆设备和非Lab分拣器到高层
   → 不克隆：Lab、Lab分拣器、传送带、喷涂机、电力感应塔
   → 每层独立地基（z=-10, 0, 10, 20）
   → 新增validateBeltLoad()验证传送带负载
...
│                              核心洞察（最新实现）                            │
│                                                                             │
│  1. init(): num缩减（除Lab外）→ ceil(60/4)=15，保存originalNum=60          │
│  2. generateBuildings(): 缩减为15个设备在z=0，Lab垂直堆叠                   │
│  3. generateConveyorBelts(): rate不放大，使用originalNum计算原料需求        │
│  4. cloneToStackLayers(): 复制15个设备×3份到高层，不克隆电力感应塔          │
│  5. 所有层共享z=0传送带网络                                                  │
│  6. z=0传送带支撑所有层总产能（通过originalNum计算）                         │
│  7. 每层独立地基（z=-10, 0, 10, 20）                                        │
│  8. Lab特殊处理：num不缩减，垂直堆叠，不克隆                                 │
│  9. 电力感应塔只在z=0层，可为所有层供电                                      │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
init() {
    ...
    // init(): num=ceil(60/4)=15, originalNum=60
...
flowchart TD
    ...
    C --> D["Step 3: 使用 originalNum 计算原料"]
    D --> E["Step 4: 遍历物料"]
    E --> F["Step 5: 选择传送带等级"]
    F --> G["Step 6: 生成分拣器连接"]
    G --> H["Step 7: 生成传送带节点"]
...
{
  itemName: {
    rate: Number,           // 产出速率（单层速率，不放大）
    inputRate: Number,      // 消耗速率（使用originalNum计算）
...
flowchart TD
    ...
    E --> F{"有 input?"}
    F -->|是| G["遍历 inputItem"]
    G --> H["计算 inputRate<br/>（使用 originalNum）"]
    H --> I["更新 itemSummary"]
    F -->|否| J["下一个 subRecipe"]
    I --> J
```
...
### 6.6 Step 3: 原料计算逻辑（核心）

**代码位置**: `blueprint.js` L2360-2420
...
```mermaid
flowchart TD
    A["堆叠模式原料计算"] --> B["获取 originalNum"]
    B --> C["使用 originalNum 计算 inputRate"]
    C --> D["传送带按总产能设计"]
    D --> E["所有层共享 z=0 传送带"]
```
...
```
设备（stackLayers=4）：
  缩减后：num=15, originalNum=60
  inputRate = baseRate × 60（使用originalNum）
  → 传送带按 60 设备总产能设计
  → z=0 传送带汇聚所有层产物

Lab（stackLayers=4）：
  num=4（不缩减，无originalNum）
  inputRate = LabRate × 4
  → Lab 传送带按 4 组 Lab 产能设计
```

### 6.8 Step 4: 分拣器分配策略

**代码位置**: `blueprint.js` L2435-2445
...
flowchart TD
    ...
    B -->|是| C["sortersPerNode = max(2, floor((max-1)/stackLayers))"]
    B -->|否| D["sortersPerNode = max"]
    C --> E["每节点分配 max(2, floor(7/4))=2 个分拣器"]
    D --> F["每节点分配 8 个分拣器"]
    E --> G["克隆后：2×4=8 ≤ 8 不超限"]
```
...
1. 原料计算
   ...
   - inputRate = baseRate × originalNum
   - 传送带按总产能设计

2. 分拣器分配
   - sortersPerNode = max(2, floor((max-1)/stackLayers))
   - z=0层每节点分配更少分拣器
   - 克隆后总数不超过限制
...
cloneToStackLayers() {
    ...
    const cloneableBuildings = baseBuildings.filter((b) => {
      ...
      if (b.itemId === teslaTowerItemId) return false;  // 电力感应塔不克隆
      return true;
    });
...
**注意**：地基z坐标比设备z坐标低10格，因为设备需要建在地基上。
...
```
z=30: [设备] [设备] [设备]  ← 克隆层（地基z=20）
         ↓      ↓      ↓
z=20: [设备] [设备] [设备]  ← 克隆层（地基z=10）
         ↓      ↓      ↓
z=10: [设备] [设备] [设备]  ← 克隆层（地基z=0）
         ↓      ↓      ↓
z=0:  [设备] [设备] [设备]  ← 原始层（地基z=-10）
       ═══════════════════  ← 共享传送带
...
// layer=0: 地基 z=-10, index=201 → z=0层设备使用
// layer=1: 地基 z=0,   index=202 → z=10层设备使用
// layer=2: 地基 z=10,  index=203 → z=20层设备使用
// layer=3: 地基 z=20,  index=204 → z=30层设备使用

// 设备连接地基：
// z=0层设备:  inputObjIdx = 201（foundationStartIndex + 0，地基z=-10）
// z=10层设备: inputObjIdx = 202（foundationStartIndex + 1，地基z=0）
// z=20层设备: inputObjIdx = 203（foundationStartIndex + 2，地基z=10）
// z=30层设备: inputObjIdx = 204（foundationStartIndex + 3，地基z=20）
```
...
| ----------------------- | ------------------------ |
| init()                  | num 不缩减，保持完整             |
| generateBuildings()     | 正常布局，垂直堆叠到maxLabLayers   |
| generateConveyorBelts() | 使用实际num计算，不使用originalNum |
...
1. init(): Lab.num 不缩减
   → 60个Lab保持60个，无originalNum

2. generateBuildings(): Lab 正常垂直堆叠
   → 60个Lab分布在z=0,3,6...42（按maxLabLayers分组）

3. generateConveyorBelts(): Lab 使用实际num计算
   → Lab不使用originalNum，使用实际num
   → 传送带按实际Lab产能设计

4. cloneToStackLayers(): Lab 不克隆
   → Lab已在generateBuildings阶段垂直堆叠
   → Lab产能完整，不损失
```

### 10.8 Lab 过滤逻辑

**代码位置**: `blueprint.js` L2120-2145
...
| 阶段                      | 不堆叠模式     | 堆叠模式                            |
| ----------------------- | --------- | ------------------------------- |
| init()                  | num 不缩减   | num ÷ stackLayers，保存originalNum |
| generateBuildings()     | 所有设备在 z=0 | 缩减后设备在 z=0                      |
| generateConveyorBelts() | 使用num计算   | 使用originalNum计算原料               |
...
| calculateBlueprintArea | L1038 | 蓝图面积计算    |
| calculateBuildingArea  | L967  | 建筑面积计算    |
| newProductionBuilding  | L1544 | 生成生产建筑    |
| init                   | L2052 | 初始化与num缩减 |
| cloneToStackLayers     | L2105 | 建筑克隆到高层   |
| validateBeltLoad       | L2260 | 传送带负载验证   |
| sortItemSummary        | L2245 | 物料排序      |
| generateConveyorBelts  | L2320 | 传送带网络生成   |
| generateBuildings      | L2724 | 建筑生成      |
