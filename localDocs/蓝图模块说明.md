    ...
    if (itemSummary[inputItem.name]) {
      ...
      itemSummary[inputItem.name] = {
        ...
        inputRate:
          ...
          productionSpeed *
          num *
          (subRecipe.acceleratorMode === 1 ? extra_rate : 1),
        fromBuildingNum: 0,
...
**代码位置**: `blueprint.js` L2110-2127`

```javascript
// 生成地基（每层独立地基）
// 地基索引计算：foundationStartIndex + layer
// layer=0 → z=-10 地基
// layer=1 → z=0 地基（z=0层设备连接）
// layer=2 → z=10 地基
// layer=3 → z=20 地基
...
2. 每层设备连接到该层对应的地基
3. 传送带网络按总产能设计（rate × stackLayers）
  ...
  subRecipes: [
    {
      ...
      output: [{ name: 'quantumChip', rate: 0.25 }],
...
// 3. 创建蓝图实例
const blueprint = new Blueprint('量子芯片生产线', [itemMap.quantumChip.iconId], recipe, config)
...
// 堆叠模式下的特殊处理：
// 1. init(): num缩减为 ceil(num/4)
// 2. generateBuildings(): 只生成 ceil(num/4) 个设备
// 3. generateConveyorBelts(): rate放大4倍
// 4. cloneToStackLayers(): 克隆设备到z=10,20,30
```
...
// 例如：stackLayers=4, max=8 → z=0 每节点 2 个分拣器 → 克隆后 2×4=8 ≤ 8
```
...
| x_y_ratio           | calculateBlueprintArea()                                       | 蓝图长宽比                 |
