...
**代码位置**: `blueprint.js` L1800-2050

```javascript
// 输出分拣器生成
for (let outputItem of subRecipe.output) {
    // 计算实际速率
    let actual_rate = outputItem.rate * 
        buildingMap[subRecipe.building.name].productionSpeed * 
        actual_building_num * extra_rate;
...
```javascript
// 不堆叠模式：stackLayers = 1
// itemSummary 使用原始 num 计算，rate 不需要放大
const stackLayers = this.config.stackLayers || 1;

// 堆叠模式使用 originalNum 计算原料需求，而非 rate 放大
// 不堆叠模式下 originalNum 不存在，使用 num 即可
const actualNum = subRecipe.building.originalNum || subRecipe.building.num;

// 不堆叠模式：actualNum = num，rate 保持原始值
// 堆叠模式：actualNum = originalNum（原始设备数量）
```
...
// 堆叠模式：sortersPerNode = Math.max(2, Math.floor((max - 1) / stackLayers))
```
...
**代码位置**: `blueprint.js` L2480-2485, L2650-2655
...
| generateConveyorBelts() | 使用 num 计算 | 使用 originalNum 计算 |
...
| 设备数量 | 60 | 15 × 4 = 60 |
| 单设备分拣器 rate | 1 | 1 |
| 分拣器 rate 总和 | 60 × 1 = 60 | 15 × 1 = 15 |
| itemSummary.rate | 60 | 使用originalNum=60计算 |
...
**堆叠模式**：
```
init(): num = ceil(60/4) = 15（缩减），originalNum = 60
generateConveyorBelts():
  actualNum = originalNum = 60
  outputRate = outputItem.rate × productionSpeed × actualNum × extra_rate
             = 1 × 1 × 60 × 1 = 60
  itemSummary.rate = 60
```
...
堆叠模式（stackLayers=4）：
  - 设备数量 = 60/4 = 15（每层）
  - 分拣器数量 = 15（每层）
  - 每节点分拣器数 = Math.max(2, Math.floor((maxSorterNumOneBelt - 1) / stackLayers))
                   = Math.max(2, Math.floor(7 / 4)) = 2
  - 节点数 = ceil(15/2) = 8个节点
...
克隆层（z=10,20,30）：
  - 分拣器随设备一起克隆
  - index自动重映射
  - 克隆后每节点分拣器数 = 2 × 4 = 8 ≤ maxSorterNumOneBelt
```
...
if (buildingMap[ownerName].category === productionCategory.smelter &&
...
| 函数名 | 行号 | 功能 |
|--------|------|------|
| mapRecipeID | L639 | 配方ID映射 |
| calculateBlueprintArea | L1038 | 蓝图面积计算 |
| calculateBuildingArea | L967 | 建筑面积计算 |
| newProductionBuilding | L1544 | 生成生产建筑 |
| init | L2052 | 初始化 |
| cloneToStackLayers | L2105 | 建筑堆叠（不堆叠模式直接返回） |
| sortItemSummary | L2285 | 物料排序 |
| generateConveyorBelts | L2320 | 传送带网络 |
