...
### 15.5 传送带被拆分成多条独立传送带（2026-02-12）

#### 问题描述
- 要求产量 120min，堆叠 4 层后最后 60min 输出
- 相同产物的流水线被拆分成了两条独立传送带
- 导致一边产物堆积，另一边拿不到东西

#### 问题根因
`generateConveyorBelts()` 中 `rate *= stackLayers` 导致传送带数量过多。

**代码位置**: `blueprint.js` L2375-2397
...
if (stackLayers > 1) {
  for (let key in itemSummary) {
    itemSummary[key].rate *= stackLayers;  // 120 × 4 = 480
  }
}
```
...
放大后：480 items/min
       ↓ 传送带生成循环
for (totalDoneRate = 0; item.rate - totalDoneRate > zero; )
  - 第1条传送带：处理 30 items/s
  - 第2条传送带：处理 30 items/s
  - ...（共 16 条独立传送带）
       ↓
结果：相同产物的传送带被拆分成多条独立传送带
```
...
传送带生成：
  - 第1条传送带：处理 30 items/s
  - 第2条传送带：处理 30 items/s
  - 第3条传送带：处理 30 items/s
  - 第4条传送带：处理 30 items/s
  - 共 4 条，合并成一条传送带网络
       ↓
结果：所有产物汇聚到一条传送带网络
```
