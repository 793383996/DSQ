...
<tr v-for="(item, index_item) in items" v-bind:class="item.rowClass">
  ...
  <td>
    <div class="ms">
      <a v-for="a in item.accValue" v-bind:class="a.class" v-bind:href='a.href'>
        {{a.showName}}
      </a>
    </div>
...
.control-bar { animation: fadeIn 0.3s ease-out; }
.list { animation: fadeIn 0.4s ease-out; }
...
@media (max-width: 768px) {
    ...
    .list { margin: 10px 5px; padding: 10px 15px; }
...
function selectPf(name, value) {
    ...
    try {
        ...
        if (e.name == "RangeError") {
            cocoMessage.warning("无法切换到X射线裂解(制氢)/重整精炼(制精炼油)公式？请尝试先将对应的另一条默认公式切换为其他公式。", 6000);
        }
        throw e;
    }
    saveSettingPf();
